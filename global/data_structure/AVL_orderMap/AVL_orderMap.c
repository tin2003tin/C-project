...
void _t_AVL_ordermap_displayNodeMaxToMin(struct T_AVL_OrderMap_Node *node, T_DisplayFunc displayFunc1, T_DisplayFunc displayFunc2);
void _t_AVL_ordermap_displayNodeBreadth(struct T_AVL_OrderMap_Node *node, T_DisplayFunc displayFunc1, T_DisplayFunc displayFunc2);
void _t_AVL_ordermap_destroyNode(struct T_AVL_OrderMap_Node *node);
...
int t_AVL_ordermap_insert(T_AVL_OrderMap *map, void *key, void *value)
{
    ...
    if (_t_AVL_ordermap_findEqualNode(map, map->head, key) != NULL)
    {
        ...
        fprintf(stderr, "Error: The key is already in map\n");
    ...
    else
    {
        ...
        if (node == NULL)
        {
            fprintf(stderr, "Error: Error to create Node\n");
            return T_AVL_ORDERMAP_ERROR;
        }
...
int t_AVL_ordermap_edit(T_AVL_OrderMap *map, void *key, void *value)
{
    ...
    if (target == NULL)
    {
        fprintf(stderr, "Error: The key does not be found in the map\n");
...
T_Pair *t_AVL_ordermap_min(const T_AVL_OrderMap *map)
{
    ...
    if (map->size == 0)
    {
        fprintf(stderr, "Error: The map is empty.\n");
...
void t_AVL_ordermap_display(const T_AVL_OrderMap *map, T_DisplayFunc displayFunc1, T_DisplayFunc displayFunc2)
{
    printf("{ ");
    _t_AVL_ordermap_displayNodeMaxToMin(map->head, displayFunc1, displayFunc2);
    printf("}\n");
}
void t_AVL_ordermap_displayBreadth(const T_AVL_OrderMap *map, T_DisplayFunc displayFunc1, T_DisplayFunc displayFunc2)
{
    printf("{ ");
...
void _t_AVL_ordermap_displayNodeBreadth(struct T_AVL_OrderMap_Node *node, T_DisplayFunc displayFunc1, T_DisplayFunc displayFunc2)
{
    ...
    T_Queue queue = t_queue_init(sizeof(struct T_AVL_OrderMap_Node *));
    t_queue_enqueue(&queue, &node);

    while (!t_queue_isEmpty(&queue))
    {
        struct T_AVL_OrderMap_Node *current = NULL;
        current = *(struct T_AVL_OrderMap_Node **)t_queue_front(&queue);
