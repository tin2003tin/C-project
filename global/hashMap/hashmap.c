...
void t_hashmap_destroy(T_HashMap *hashmap)
{
    ...
    for (size_t i = 0; i < hashmap->bucketsLength; i++)
    {
        ...
        {
            if (hashmap->buckets[i].vector.data != NULL)
            {
                T_Pair *pair = (T_Pair *)(t_vector_iter_at(&hashmap->buckets[i].vector, j).pointer);
                if (pair != NULL)
                {
                    T_pair_destroy(pair);
                }
        ...
        t_vector_destroy(&(hashmap->buckets[i].vector));
    }
...
int t_hashmap_edit(T_HashMap *hashmap, void *key, void *value)
{
    ...
    {
        printf("\x1B[31m"
               "The key does not be found in the hashMap\n"
               "\x1B[0m");
        return T_HASHMAP_ERROR;
    }
...
int _t_hashmap_init_buckets(T_HashMap *hashmap)
{
    ...
    {
        printf("\x1B[31m"
               "Failed to reallocate memory for buckets.\n"
               "\x1B[0m");
        return T_HASHMAP_ERROR;
    }
...
int _t_HashMap_expand(T_HashMap *hashmap)
{
    ...
    {
        printf("\x1B[31mFailed to reallocate memory for expand.\n\x1B[0m");
        return T_HASHMAP_ERROR;
    }
